...
def build_cmd_args() -> argparse.Namespace:
    ...
    To run execute_ws.py, we will require two arguments, contract_ID and request_ID.
    Example: python execute_ws.py --contract_ID VRMC_OPS --request_ID 22E82CDB-420D-4D98-ACE5-AA59728F59FE
    """
    
    parser = argparse.ArgumentParser(description="Execute the whole warm start work flow.")
...
def extract(contract_ID: str, request_ID: str):
    ...
    files = ["v_GetOptimiseInspectionInputItems.parquet", "v_DomainValueDepot.parquet", 
             "v_GetOptimiseInputCrews.parquet", "v_GetDistTimeCacheForOptRun.parquet"] # inspection list, depot list, crew list, matrix data
    ...
    crew_input = pd.read_parquet(os.path.join(dst_directory, files[2]))
...
if __name__ == '__main__':
    ...
    subprocess.run([python_exe, "preprocessing_ws.py", "--request_ID", f"{request_ID}", "--s_date", f"{s_date_str}", '--s_time', f'{s_time_str}'])
    print('\nBooting Julia...')
    subprocess.run([julia_exe, "optimise_ws.jl", "--request_ID", f"{request_ID}", "--s_date", f"{s_date_str}", '--s_time', f'{s_time_str}'])
    subprocess.run([python_exe, "plot_ws.py", "--request_ID", f"{request_ID}"]) 
    subprocess.run([python_exe, "load_ws.py", "--request_ID", f"{request_ID}", "--contract_ID", f"{contract_ID}"])  
